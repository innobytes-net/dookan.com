...
def sendmail():
	...
	receiver_email = request.form['mail']  #"saakshiagrawal55@gmail.com"
...
def search():
    if request.method == "POST":
        cursor.executemany('''select * from addpro where pname = %s''', request.form['search'])
...
def addpro():
    ...
    prodes = request.form.get('prodes')
    qty = request.form.get('qty')
    price = request.form.get('price')
    myCategory = request.form.get('myCategory')
    myfile = request.form.get('myfile')

    cursor=db.cursor()
    cursor.execute("""INSERT INTO `addpro` (`pname`, `prodes`, `myCategory`, `qty`, `price`, `myfile`) VALUES ('{}', '{}', '{}', '{}', '{}', '{}')""".format(pname, prodes, myCategory, qty, price, myfile))
    conn=mysql.connect()
    conn.commit()
    cursor.execute("""SELECT * FROM addpro""")
    data = cursor.fetchall()
    return render_template('addpro.html', value = data)
